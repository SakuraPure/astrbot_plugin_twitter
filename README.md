...
### 🗣️ 自然语言调用
- **LLM 函数工具** — 关注/取关/查列表/开关推送/看最新推文均可在 AI 对话中用自然语言触发，详见下方指令表
...
在 AI 对话中可直接用自然语言触发以下功能，无需记忆指令（需 AstrBot 已配置 LLM Provider 且模型支持函数调用）：

| 工具 | 功能 | 示例说法 |
|------|------|----------|
| `twitter_follow_user` | 订阅推主（支持 R18 / 仅媒体选项） | "帮我关注一下 elonmusk 的推特" |
| `twitter_unfollow_user` | 取关推主 | "取关 elonmusk" |
| `twitter_list_subscriptions` | 查看当前会话订阅列表 | "我都关注了哪些推主？" |
| `twitter_toggle_push` | 开关当前会话推送 | "暂停推特推送" / "恢复推送" |
| `twitter_fetch_latest_tweet` | 获取最新推文（可按图片/视频过滤），推文直接发到会话 | "看看 elonmusk 的最新推文" / "发一张 xxx 最新的图" |

> 管理类高危操作（如 `/推特清空订阅`）未暴露给 LLM，仍需管理员通过指令执行。
